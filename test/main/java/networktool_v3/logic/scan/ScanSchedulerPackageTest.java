...
class ScanSchedulerPackageTest {
    ...
    static final String PROFILE = "__sched_pkg_test__";
    ...
    @Test
    void start_knownProfile_isRunning() throws InterruptedException {
    ...
    @Test
    void getRunning_containsStarted() throws InterruptedException {
        sched.start(PROFILE, 999, "");
        Thread.sleep(100);
    ...
    @Test
    void startTwice_replacesExisting() throws InterruptedException {
        sched.start(PROFILE, 999, "");
        Thread.sleep(50);
        sched.start(PROFILE, 999, "");
        Thread.sleep(50);
        assertEquals(1, sched.getRunning().stream()
                .filter(PROFILE::equals).count());
    }
