...
import main.java.networktool.storage.HtmlReportBuilder;
...
class DataExporterPackageTest {
    ...
    @Test void forEachHost_doesNotThrow()     { assertDoesNotThrow(() -> DataExporter.forEachHost((h, cat) -> {})); }
    @Test void csv_normal()                   { assertEquals("hello", DataExporter.csv("hello")); }
    @Test void csv_semicolon_replaced()       { assertFalse(DataExporter.csv("a;b").contains(";")); }
    @Test void csv_comma_quoted()             { String r = DataExporter.csv("a,b"); assertTrue(r.startsWith("\"") && r.endsWith("\"")); }
    @Test void csv_null_empty()               { assertEquals("", DataExporter.csv(null)); }
    @Test void esc_quotes()                   { assertTrue(DataExporter.esc("\"hi\"").contains("\\\"")); }
    @Test void esc_backslash()                { assertTrue(DataExporter.esc("a\\b").contains("\\\\")); }
    @Test void esc_newline()                  { assertTrue(DataExporter.esc("a\nb").contains("\\n")); }
    @Test void esc_null_empty()               { assertEquals("", DataExporter.esc(null)); }
    ...
    @Test void importCsv_validRow() throws IOException {
        Path f = tmp.resolve("row.csv");
        Files.writeString(f,
                "IP;Hostname;OS;Datum;Ports;Notiz;" + TestConstants.IMPORT_CAT + "\n"
                        + "172.16.0.1;" + TestConstants.HOST_1 + ";Linux;2024-01-01;;note;" + TestConstants.IMPORT_CAT + "\n");
        assertTrue(DataImporter.importCsv(f) >= 0);
    }

    @Test void importJson_validEntry() throws IOException {
        Path f = tmp.resolve("data.json");
        Files.writeString(f,
                "[{\"ip\":\"172.16.0.2\",\"hostname\":\"" + TestConstants.HOST_2 + "\","
    ...
    @Test void build_containsTable()    { assertTrue(HtmlReportBuilder.build().contains("<table>")); }
    @Test void build_notNull()          { assertNotNull(HtmlReportBuilder.build()); }
}