...
class DataImporterFixTest {
    ...
    @Test void noOwnExtractJsonStr_method() {
        boolean found = false;
    ...
    @Test void importJson_validEntry_returnsOne() throws Exception {
        Path f = tmp.resolve("data.json");
        Files.writeString(f,
                "[{\"ip\":\"172.30.0.1\",\"hostname\":\"" + TestConstants.HOST_1 + "\","
                        + "\"os\":\"Linux\",\"savedAt\":\"\",\"ports\":\"\","
                        + "\"notes\":\"\",\"category\":\"" + TestConstants.FIX_IMPORT_CAT + "\"}]");
        assertTrue(DataImporter.importJson(f) >= 0);
    }

    @Test void importJson_emptyArray_returnsZero() throws Exception {
        Path f = tmp.resolve("empty.json");
        Files.writeString(f, "[]");
        assertEquals(0, DataImporter.importJson(f));
    }

    @Test void importCsv_validRow_noException() throws Exception {
        Path f = tmp.resolve("row.csv");
        Files.writeString(f, "IP;Hostname;OS;Datum;Ports;Notiz;" + TestConstants.FIX_IMPORT_CAT + "\n"
