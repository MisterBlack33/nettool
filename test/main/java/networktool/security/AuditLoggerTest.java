...
class AuditLoggerTest {
    ...
    // ── AuditLogger API ──────────────────────────────────────────────────
    // Shares "userAuthSingleton" lock with SecurityTest/MainTest/UserAuthPasswordPolicyTest:
    // this class logs in as real UserAuth users (needed for clear()'s admin check), so it
    // must not run concurrently with any other class mutating that same singleton.
