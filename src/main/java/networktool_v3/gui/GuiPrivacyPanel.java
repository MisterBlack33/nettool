 ...
 *
 * Funktionen:
 *  - MAC-Adresse des aktiven Interfaces randomisieren / zurücksetzen
 *  - WireGuard-VPN starten / stoppen (wenn wireguard-tools installiert)
 *  - Lokale IP anzeigen + Hinweis zum Verschleiern per VPN/Proxy
 *  - Netzwerk-Traffic-Verschlüsselung: Status-Check
 *
 * Alle Aktionen laufen asynchron. Root/Admin-Rechte können erforderlich sein.
 */
public final class GuiPrivacyPanel {
    ...
    private static void embedPanel(GuiOutputPanel output) {
        ...
        macRandBtn.addActionListener(e ->
                runPrivacyTask(logArea, "MAC randomisieren", GuiPrivacyPanel::randomizeMac));
        macResetBtn.addActionListener(e ->
                runPrivacyTask(logArea, "MAC zurücksetzen", GuiPrivacyPanel::resetMac));
        vpnStartBtn.addActionListener(e ->
                runPrivacyTask(logArea, "WireGuard starten", GuiPrivacyPanel::startVpn));
        vpnStopBtn.addActionListener(e ->
                runPrivacyTask(logArea, "WireGuard stoppen", GuiPrivacyPanel::stopVpn));
        checkEncBtn.addActionListener(e ->
                runPrivacyTask(logArea, "Verschlüsselung prüfen", GuiPrivacyPanel::checkEncryption));
        ...
        new Thread(() -> {
            ...
            SwingUtilities.invokeLater(() -> {
                macLabel.setText("MAC: " + mac);
                macLabel.setForeground(FG);
    ...
    private static void randomizeMac(JTextArea log) {
        ...
        if (iface == null) { log("MAC: Kein aktives Interface gefunden.", log); return; }
    ...
    private static void runPrivacyTask(JTextArea log, String name, java.util.function.Consumer<JTextArea> task) {
        log.setText("");
        log("── " + name + " ──", log);
    ...
    private static String getCurrentMac() {
        try {
            java.net.NetworkInterface ni = java.net.NetworkInterface.getByName(getActiveInterface());
